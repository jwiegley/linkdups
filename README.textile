...
Here’s how the algorithm works:
 ...
 # First, I divide the problem between large files (easy gains), and small
 ...
 # Next, a table is created of how big each file in the target set (large or
   small) is. Groups are then made for like-size files; singletons are
   dropped.
 # Within the like-size groups, MD5 is used for a quick content check. Files
   in a size group with matching checksums are considered candidates for
   linking.
 # The candidates from step 4 are byte-wise compared to ensure an exact
 ...
 # At the end of the run — or after Control-C has been pressed — the total
